...
void open_settings_menu(global_t *all, g_menu *gmenu)
{
    ...
    while (all->settings_menu_state == 1) {
        ...
        render_several(menu.window, 3, menu.s_menu_background, menu.sound_bar, menu.sound_icon);
...
void s_buttons_actions(s_menu *menu, sfRenderWindow *window, sfEvent event)
{
    ...
    //    blink_button(menu, window, menu->resume, BF_RESUME);
    //    if (event.type == sfEvtMouseButtonPressed) printf("CONTINUE !\n");
    //}
}
...
void settings_menu_event(global_t *all, s_menu *menu)
{
    if (menu->event.type == sfEvtKeyPressed &&
        ...
        destroy_several(3, menu->s_menu_background, menu->sound_bar, menu->sound_icon);
